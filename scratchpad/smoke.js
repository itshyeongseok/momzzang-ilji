...
/* ========== 신규(PR-2): 수면 입력 UI(투데이 카드 + 프로필 섹션) ========== */
t('saveTodaySleep: 오늘 날짜로 sleep에 총수면 기록',()=>{
  reset();
  ctx.document.getElementById('todaySl').value='7.5';
  ev('saveTodaySleep()');
  const rec=DB().sleep.find(s=>s.date==='2026-06-27');
  assert(rec,'오늘 날짜 행 생성');
  assert.strictEqual(rec.hours,7.5,'총수면 저장');
  assert.strictEqual(rec.rem,null,'렘 비움');
  assert.strictEqual(rec.deep,null,'깊은잠 비움');
  assert.strictEqual(rec.photo,null,'photo는 null(이미지는 PR-3)');
});

t('saveTodaySleep: 같은 날 재저장은 행 추가 없이 값만 갱신',()=>{
  reset();
  ctx.document.getElementById('todaySl').value='7.5';ev('saveTodaySleep()');
  ctx.document.getElementById('todaySl').value='8';ev('saveTodaySleep()');
  const todays=DB().sleep.filter(s=>s.date==='2026-06-27');
  assert.strictEqual(todays.length,1,'중복 행 없음');
  assert.strictEqual(todays[0].hours,8,'값 갱신');
});

t('saveTodaySleep: 빈 값/0 이하는 무시(행 변동 없음)',()=>{
  reset();
  ctx.document.getElementById('todaySl').value='7';ev('saveTodaySleep()');
  ctx.document.getElementById('todaySl').value='';ev('saveTodaySleep()');
  ctx.document.getElementById('todaySl').value='0';ev('saveTodaySleep()');
  assert.strictEqual(DB().sleep.filter(s=>s.date==='2026-06-27').length,1,'유효값 1건만');
  assert.strictEqual(DB().sleep[0].hours,7,'기존값 보존');
});

t('addSleep: 프로필 입력 — 날짜별 upsert(렘/깊은잠/메모)',()=>{
  reset();
  ctx.document.getElementById('slDate').value='2026-06-25';
  ctx.document.getElementById('slHours').value='7';
  ctx.document.getElementById('slRem').value='1.5';
  ctx.document.getElementById('slDeep').value='1.2';
  ctx.document.getElementById('slMemo').value='꿀잠';
  ev('addSleep()');
  const rec=DB().sleep.find(s=>s.date==='2026-06-25');
  assert(rec,'행 생성');
  assert.strictEqual(rec.hours,7);assert.strictEqual(rec.rem,1.5);
  assert.strictEqual(rec.deep,1.2);assert.strictEqual(rec.memo,'꿀잠');
});

t('addSleep: 같은 날짜 재저장은 병합(입력 필드만 덮어쓰기, 중복X)',()=>{
  reset();
  ctx.document.getElementById('slDate').value='2026-06-25';
  ctx.document.getElementById('slHours').value='7';
  ctx.document.getElementById('slRem').value='';
  ctx.document.getElementById('slDeep').value='';
  ctx.document.getElementById('slMemo').value='';
  ev('addSleep()');
  // 같은 날 렘만 추가
  ctx.document.getElementById('slHours').value='';
  ctx.document.getElementById('slRem').value='1.4';
  ev('addSleep()');
  const recs=DB().sleep.filter(s=>s.date==='2026-06-25');
  assert.strictEqual(recs.length,1,'중복 없음(날짜별 upsert)');
  assert.strictEqual(recs[0].hours,7,'기존 총수면 보존');
  assert.strictEqual(recs[0].rem,1.4,'렘 추가');
});

t('addSleep: 빈 입력(값 없음)은 토스트로 막음(행 없음)',()=>{
  reset();clearSpies();
  ctx.document.getElementById('slDate').value='2026-06-25';
  ctx.document.getElementById('slHours').value='';
  ctx.document.getElementById('slRem').value='';
  ctx.document.getElementById('slDeep').value='';
  ctx.document.getElementById('slMemo').value='메모만';
  ev('addSleep()');
  assert.strictEqual(DB().sleep.length,0,'값 없으면 저장 안 됨');
  assert(toastList().some(m=>m.includes('하나 이상')),'안내 토스트');
});

t('delSleep: 인덱스로 수면 기록 삭제',()=>{
  reset();
  ev("DB.sleep=[{date:'2026-06-24',hours:7,rem:null,deep:null,memo:null,photo:null},{date:'2026-06-25',hours:8,rem:null,deep:null,memo:null,photo:null}];save();");
  ev('delSleep(0)');
  assert.strictEqual(DB().sleep.length,1,'1건 삭제');
  assert.strictEqual(DB().sleep[0].date,'2026-06-25','뒤 기록 보존');
});

t('sleepAvg: 최근 7일 입력된 날만 평균(없으면 null)',()=>{
  reset();ev("cur='2026-06-27';");
  // 7일 내 2건(총수면), 렘은 1건만, 깊은잠 없음, 8일전은 범위 밖
  ev("DB.sleep=[{date:'2026-06-19',hours:10,rem:9,deep:9,memo:null,photo:null},"+
     "{date:'2026-06-25',hours:7,rem:1.5,deep:null,memo:null,photo:null},"+
     "{date:'2026-06-26',hours:8,rem:null,deep:null,memo:null,photo:null}];save();");
  assert.strictEqual(ev("sleepAvg('hours',7)"),7.5,'총수면 (7+8)/2');
  assert.strictEqual(ev("sleepAvg('rem',7)"),1.5,'렘은 입력된 1건만');
  assert.strictEqual(ev("sleepAvg('deep',7)"),null,'깊은잠 입력 없으면 null');
});

t('투데이 수면 카드: 기록 없으면 안내, 있으면 값+기록됨 태그 렌더',()=>{
  reset();
  let v=ev('viewTodaySleep()');
  assert(v.includes('오늘 수면')&&v.includes('id="todaySl"'),'카드/입력 렌더');
  assert(v.includes('숫자만 입력하면'),'빈 상태 안내');
  assert(!v.includes('기록됨'),'빈 상태엔 태그 없음');
  ev("DB.sleep=[{date:'2026-06-27',hours:7.5,rem:null,deep:null,memo:null,photo:null}];save();");
  v=ev('viewTodaySleep()');
  assert(v.includes('기록됨')&&v.includes('7.5h'),'기록 시 태그+값');
});

t('프로필 수면 섹션: 평균 요약·입력·목록 렌더 무에러(setTab me)',()=>{
  reset();
  ev("DB.sleep=[{date:'2026-06-26',hours:8,rem:1.6,deep:1.3,memo:'좋음',photo:null}];save();");
  ev("setTab('me')");
  const v=ctx.document.getElementById('view').innerHTML;
  assert(v.includes('id="sleepSection"'),'수면 섹션 앵커');
  assert(v.includes('최근 수면')&&v.includes('총수면'),'평균 요약');
  assert(v.includes('id="slHours"')&&v.includes('addSleep()'),'입력 카드');
  assert(v.includes('수면 기록 목록')&&v.includes('2026-06-26'),'기록 목록');
  assert(v.includes('delSleep('),'삭제 버튼');
});

t('프로필 수면 섹션: 기록 없으면 빈 상태(목록 미표시)',()=>{
  reset();
  const v=ev('viewSleep()');
  assert(v.includes('회복 추세가 나타나요'),'빈 상태 안내');
  assert(!v.includes('수면 기록 목록'),'기록 없으면 목록 카드 없음');
  assert(v.includes('id="slHours"'),'입력 카드는 항상 표시');
});
...
t('옛 데이터 호환: sleep 키 없는 백업 import 후 수면 렌더 무에러',()=>{
  ...
  const v=ev('viewSleep()');
  assert(v.includes('회복 추세'),'빈 수면 섹션 렌더');
  assert.strictEqual(ev("viewTodaySleep().includes('id=\"todaySl\"')"),true,'투데이 카드도 렌더');
});
