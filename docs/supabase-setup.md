...
- **Confirm email** = ON
- **Enable Email OTP / Magic Link** = ON (OTP 코드·링크 발송에 필요)
...
## (c-2) ★ OTP(6자리 코드) — 이메일 템플릿에 `{{ .Token }}` 추가 (필수 1회 편집)

> 앱 로그인은 **매직링크 → 이메일 OTP(6자리 코드)** 로 바뀌었습니다. 이유: 매직링크가 홈화면 앱이 아닌 다른 브라우저로 열려 로그인이 엉뚱한 데로 가는 문제 제거. 이제 **코드를 앱 안에서 입력**하므로 브라우저 전환이 없습니다.
>
> Supabase 기본 메일 템플릿은 **링크(`{{ .ConfirmationURL }}`)만** 들어 있어 코드가 메일에 안 보입니다. 코드가 메일에 나오려면 **템플릿에 `{{ .Token }}`(6자리 코드) 한 줄을 추가**해야 합니다. (가입 불필요 — 대시보드에서 한 번만 편집.)

대시보드 → **Authentication** → **Emails** (또는 **Email Templates**) → **"Magic Link"** 템플릿을 열고, 본문에 `{{ .Token }}` 을 포함하도록 한 줄 추가한 뒤 **Save**.

예시 템플릿(그대로 붙여넣어도 됩니다):
...
> - `{{ .Token }}` 가 핵심입니다. 링크(`{{ .ConfirmationURL }}`)는 남겨둬도 되고(혹시 링크로 와도 앱이 `detectSessionInUrl` 로 동작), 지워도 됩니다.
> - Supabase는 OTP 발송 시 동일하게 이 "Magic Link" 템플릿을 사용합니다(별도 OTP 전용 템플릿이 없음).
> - 코드 만료/길이 등은 **Authentication → Providers → Email** 의 OTP 설정(Email OTP Expiration, 기본 6자리)에서 조정 가능합니다(기본값 그대로 OK).

---

## 폰 실기기 검증 (헤드리스로는 불가)

OTP 코드 메일·실 네트워크·실제 동기화는 헤드리스 스모크로 검증할 수 없어, **폰에서 수동 확인**합니다:

1. (a)~(c)+**(c-2)** 완료 후 배포본 접속 → **프로필 탭 → "☁️ 클라우드 동기화"** 카드에 이메일 입력 → **인증코드 받기**.
2. 메일의 **6자리 코드**를 앱 카드에 입력 → **확인** → 카드가 **"● 동기화 켜짐"** 으로 바뀌는지 확인. (코드가 메일에 안 보이면 (c-2)의 `{{ .Token }}` 편집 누락 — 다시 확인.)
3. 폰A에서 운동/체중을 기록 → 다른 기기(또는 시크릿 창)에서 같은 이메일로 로그인 → **같은 데이터가 pull** 되는지 확인.
4. (선택) 비행기모드로 기록 후 복구 → 다음 저장/포커스에서 자동 재동기화 확인.
5. (선택) "코드 재전송" / "이메일 수정" 버튼이 1·2단계를 오가게 동작하는지, 한 번 로그인 후 앱 재실행에도 **로그인 유지**되는지 확인.
